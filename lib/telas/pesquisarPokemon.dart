...
import 'package:pk_tranding_card/telas/pokemonDetails.dart';
import '../data/apiData.dart';
import '../model/pokemon.dart';
import '../model/searchPokemon.dart';
import '../model/cores/listaDeCores.dart';

class ItemPage extends StatefulWidget {
  final List<SearchPokemons> pokemon;
  final String nome;
  List<Pokemons> box;
...
class ItemPageState extends State<ItemPage> {
  ...
  final int tamanho = 1;
  late String link;
  String imagemDefault = "https://logodownload.org/wp-content/uploads/2017/08/pokemon-logo-0.png";
  String imagemEncontrada = "https://upload.wikimedia.org/wikipedia/commons/5/54/Ajux_loader.gif";
  ...
  void _buscar() {
    for(var i=0; i < widget.pokemon.length; i++) {
      if (widget.pokemon[i].nome.compareTo(widget.nome) == 0) {
        setState(() {
          temNaLista = true;
          link = widget.pokemon[i].link;
          _iniciaLista();
        });
        break;
      }
  ...
  void _iniciaLista() async {
    await HttpService.getPokemon(link).then((response) {
      setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.of(context).pop(widget.box);
          },
        ),
      ),
      body: Stack(
            ...
            children: [
              temNaLista
                                        ...
                                        ElevatedButton(
                                          ...
                                          onPressed: () => {
                                            Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                builder:(context) =>PokemonDetalhes(
                                                  link: link,
                                                  box: widget.box
                                                ),
                                              ),
                ...
                :
                  Stack(
                    children: [
                      Image.network(imagemDefault, fit: BoxFit.fill,),
                      Padding(
                        padding: const EdgeInsets.only(top: 200.0),
                        child: AlertDialog(
                          title: Text("O POKÉMON NÃO FOI ENCONTRADO!",
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xff353D64),
                              fontSize: 18.0,
                          ...
                          content: Text("Verifique se o nome digitado está correto.",
                            style: TextStyle(
                              color: Color(0xff353D64),
                              fontSize: 16.0,
                            ),
